...
def BatchGemmGemm(batch, M, N, K, L, in_dtype="float16", acc_dtype="float32"):
    ...
    D_shared_half = tvm.te.compute(
        [batch, M // MI, L // NI, MI, NI],
        lambda b, mo, lo, mi, li: D_shared[b, mo, lo, mi, li].astype(
            in_dtype
        ),
        name="D_shared_half",
    )
    ...
    E_frag = tvm.te.compute(
        ...
        lambda b, mo, no, mi, ni: tvm.te.sum(
            D_frag1[b, mo, rlo, mi, rli].astype(acc_dtype)
...
def schedule_llvm(batch, M, N, K, L, in_dtype="float16", acc_dtype="float32"):
    ...
    D_frag1, C_frag = E_frag.op.input_tensors
    D_shared_half = D_frag1.op.input_tensors[0]
    ...
    sch[D_shared_half].compute_inline()
    sch[C_shared].compute_inline()
    sch[D_frag1].compute_inline()
...
def schedule_cuda(batch, M, N, K, L, in_dtype="float16", acc_dtype="float32"):
    ...
    exp_shared = D_shared_half
    sch[C_shared].set_scope("shared")
    C_ext_shared = C_shared
    ...
    sch[D_frag1].set_scope("wmma.matrix_a")
    ...
    sch[D_frag1].compute_at(sch[exp_frag_attach_tensor], exp_frag_attach_axis)
    b, mo, lo, mi, li = sch[D_frag1].op.axis
    # sch[D_frag1].reorder(b, mo, lo, mi, li)
    # tensorize mi
    sch[D_frag1].tensorize(mi, intrin_wmma_load_matrix_a("float16"))
    ...
    fused = sch[exp_shared].fuse(mo, lo, mi, li)
