...
public:
  LazyLogging() = default;
  LazyLogging(const LazyLogging &&other)
      : log_level(other.log_level), do_print(other.do_print) {}
  ...
  ~LazyLogging() {
    ...
    if (do_print) {
      ...
      case LogLevel::tWARNING:
        std::cerr << "Ditto: [Warning] "
                  << "[time=" << ms.count() << "] file:" << file_
                  << " line:" << lineno_ << " " << oss.str() << std::flush;
        break;
      case LogLevel::tERROR: {
        std::cerr << "Ditto: [Error] "
                  << "[time=" << ms.count() << "] " << file_
                  << " line:" << lineno_ << " " << oss.str() << std::flush;
        abort();
      } break;
  ...
  template <typename T> LazyLogging &operator<<(T &other) {
    oss << other;
    return *this;
  }
...
public:
  print(int level, std::ostream &out = std::cerr)
      : do_print(level == 0 || level <= get_evn_value("DITTO_PRINT_LEVEL")),
        out(out) {}

  template <typename T> print &operator<<(T &&x) {
    if (do_print) {
