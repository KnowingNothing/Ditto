...
public:
   ...
   * 0. register the searchSpace in alg; register the iterGraph in evals (move to build SearchDriver)
...
public:
  TVM_DLL SearchDriver(Array<Evaluator> evals, SearchSpace searchSpace, \
          SearchAlg alg);
  TVM_DEFINE_MUTABLE_OBJECT_REF_METHODS(SearchDriver, ObjectRef, SearchDriverNode);
