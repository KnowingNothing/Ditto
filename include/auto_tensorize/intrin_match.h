...
private:
  ...
  bool _match(Tensor target, Operation capsule,
              ...
              Map<IterVar, Range> intrin_bounds, bool innermost = false,
  ...
  // bool _check_elemwise(const ComputeOpNode* op, Array<Array<PrimExpr>>&
  // indices);
};

class CapsuleExprMatcher
    : public ExprFunctor<bool(const PrimExpr &, const PrimExpr &)> {
public:
  using ExprFunctor::VisitExpr;
  CapsuleExprMatcher(BufferMap &bm) : buffer_map(bm){};
  Array<IterVarMap> match(PrimExpr target, PrimExpr intrin,
                          Array<IterVar> &target_axes,
                          Array<IterVar> &intrin_axes,
                          Map<IterVar, Range> target_bounds,
                          Map<IterVar, Range> intrin_bounds);
  void extract_indices(PrimExpr target, PrimExpr intrin,
...
#define MATCH(T)                                                               \
  ...
  template <typename T> bool VisitBinary(const T *op, const PrimExpr &expr) {
    MATCH(T)
...
public:
  ...
  IterVarRewriter(IterVarMap &itervar_map, Map<IterVar, Range> &bounds)
