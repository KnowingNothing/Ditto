...
from ditto.auto_compute.nn.model import CapsNet


def train_fp32(test=False):
    A = ac.layer_tensor([1, 1, 28, 28], dtype="float32", name="A")
    model = CapsNet()
    outputs = model(A)
    

    graph = ac.graph([A], outputs)
    grad_graph = ag.grad_graph(graph, reserve_forward=False)
    print(grad_graph)
    ...
    target_host = "llvm"
    trials = 10000
    task_name = "capsule_lenet_train_fp32"
    log_file = "capsule_lenet_train_fp32.log"
    builder = "local"
    runner = "local"
