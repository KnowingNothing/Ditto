...
from ditto.auto_compute.nn.model import MI_LSTM


def train_fp32(test=False):
    A = ac.layer_tensor([1, 28 * 28], dtype="float32", name="A")
    B = ac.layer_tensor([1, 1024], dtype="float32", name="B")
    C = ac.layer_tensor([1, 1024], dtype="float32", name="C")
    model = MI_LSTM()
    outputs, h, c = model(A, B, C)
    

    graph = ac.graph([A, B, C], [outputs, h, c])
    ...
    target_host = "llvm"
    trials = 10000
    task_name = "MI_LSTM_train_fp32"
    log_file = "MI_LSTM_train_fp32.log"
    builder = "local"
    runner = "local"
