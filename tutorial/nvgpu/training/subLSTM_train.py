...
from ditto.auto_compute.nn.model import subLSTM


def train_fp32(test=False):
    A = ac.layer_tensor([1, 28 * 28], dtype="float32", name="A")
    B = ac.layer_tensor([1, 128], dtype="float32", name="B")
    C = ac.layer_tensor([1, 128], dtype="float32", name="C")
    model = subLSTM()
    ...
    target_host = "llvm"
    trials = 10000
    task_name = "subLSTM_train_fp32"
    log_file = "subLSTM_train_fp32.log"
    builder = "local"
    runner = "local"
