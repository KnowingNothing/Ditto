...
// #define systolic_config_execute(...) GET_MACRO(__VA_ARGS__, systolic_config_execute_bf, , systolic_config_execute, )(__VA_ARGS__)
...
#define systolic_config_dataflow_type(dataflow_type) \
  ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC,(uint64_t)(dataflow_type), (uint64_t)(0), k_CONFIG_DATAFLOW)

#define systolic_config_execute(padding_size,kernel_size , out_size, ctrl, sgn) \
  ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC,(uint64_t)(kernel_size)|((uint64_t)ctrl << 32)|((uint64_t)sgn << 34)|((uint64_t)(padding_size) << 8), (uint64_t)(out_size), k_CONFIG_EXEC)


#define systolic_config_slot_size(input_slot_size,out_slot_size,filter_slot_size) \
  ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC,((uint64_t)(out_slot_size) << ADDR_LEN) | (input_slot_size), (uint64_t)(filter_slot_size), k_CONFIG_SLOT)

#define systolic_config_activation(act_func,act_param) \
  ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC,(uint64_t)(act_func), (uint64_t)(act_param), k_CONFIG_ACTI)

#define systolic_padding_data(padding_left,padding_data) \
  ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC,(uint64_t)(padding_data) | (uint64_t)((padding_left) << 16), (uint64_t)(0), k_CONFIG_PADDING_DATA)

#define systolic_config_stride(conv_stride) \
  ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC,(uint64_t)(conv_stride), (uint64_t)(0), k_CONFIG_STRI)

#define systolic_pool_func(pool_func) \
  ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC,(uint64_t)(pool_func), (uint64_t)(0), k_CONFIG_POOL_FUNC)

#define systolic_pool_param(rs1,rs2) \
  ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC,(uint64_t)(rs1), (uint64_t)(rs2), k_CONFIG_POOL_PARA)

#define systolic_sram_cfg(rs1,rs2) \
  ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC,(uint64_t)(rs1), (uint64_t)(rs2), k_CONFIG_SRAM_BYPASS)

// sel_mem = 0: input buffer 
// sel_mem = 1: result buffer 
#define systolic_mvin_input(dram_addr, slot_id, sel_mem) \
  ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC, dram_addr, ((uint64_t)(sel_mem) << ADDR_LEN) | (slot_id), k_MVIN_INPUT)

#define systolic_mvin_filter(dram_addr, slot_id  ) \
  ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC, dram_addr,  ((uint64_t)(0) << ADDR_LEN) | (slot_id), k_MVIN_FILTER)

// mv_out
#define systolic_mvout(dram_addr, slot_id , sel_mem) \
  ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC, dram_addr,  ((uint64_t)(sel_mem) << ADDR_LEN) | (slot_id), k_MVOUT)

#define systolic_conv_pre(input_slot_id, input_sel_mem, filter_slot_id ) \
  ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC,  ((uint64_t)(input_sel_mem) << ADDR_LEN) | (input_slot_id), filter_slot_id,  k_CONV_PRE)

#define systolic_conv_exec(out_slot_id , out_sel_mem , zeroize,zero_direction,tiling) \
  ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC, ((uint64_t)(out_sel_mem) << ADDR_LEN) | (uint64_t)(out_slot_id), (uint64_t)(zeroize) | (uint64_t)(zero_direction) << 16| ((uint64_t)(tiling) << 32),  k_CONV_EXEC)

#define systolic_postproc(reg_type,reg_addr , reg_data) \
  ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC,  (uint64_t)(reg_addr) | (uint64_t)(reg_type) << 8, (uint64_t)(reg_data) ,  k_POSTPROC)

#define systolic_postproc_offset(mutil_shift , out_offset) \
  ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC,   (uint64_t)(3) << 8, (uint64_t)mutil_shift << 32 | (uint64_t)((uint32_t)(out_offset)) ,  k_POSTPROC)

#define systolic_leakyrelu(leakrelu_addr ,leakrelu_data ) \
  ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC,   (uint64_t)(leakrelu_addr), (uint64_t)(leakrelu_data) ,  k_CONFIG_LEAKY)

#define systolic_fence() \
  ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC, 0,  0, k_FENCE)
...
#define gemmini_extended_mvin(dram_addr, slot_id, cols, rows) \
  ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC, dram_addr,  ((uint64_t)(cols) << ADDR_LEN) | (slot_id), 0)
