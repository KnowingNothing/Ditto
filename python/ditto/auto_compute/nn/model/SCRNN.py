...
class InternalLayer1(Module):
    ...
    def forward(self, inputs, state_c):
        ...
        context_state = tvm.te.compute(
            [batch, context_units],
            lambda i, j: (1-self.alpha) *
            inputs[i, j] + self.alpha * state_c[i, j],
            name="state_c"
        )

        d_layer = layer(context_state.op, inputs=[inputs.tensor, state_c.tensor],
                        weights=None,
                        const_scalars=None,
                        const_tensors=None,
...
class ConcatLayer(Module):
    ...
    def forward(self, context_state, inputs, state_h):
        context_state, inpust, state_h = self.preprocess(
            context_state, inputs, state_h)
        ...
        d_layer = layer(concated.op, inputs=[context_state.tensor, inputs.tensor, state_h.tensor],
                        weights=None,
                        const_scalars=None,
                        const_tensors=None,
                        requires_grad=self.training,
...
class InternalLayer2(Module):
    ...
    def forward(self, fc):
        ...
        outputs = tvm.te.compute(
            [N, H],
            lambda i, j: tvm.te.sigmoid(fc[i, j]),
            name="sigmoid"
        )

        d_layer = layer(outputs.op, inputs=[fc.tensor],
                        weights=None,
                        const_scalars=None,
                        const_tensors=None,
                        requires_grad=self.training,
...
class InternalLayer3(Module):
    ...
    def forward(self, h, c):
        ...
        outputs = tvm.te.compute(
            [batch, num_units],
            lambda i, j: h[i, j] + c[i, j],
            name="new_h"
        )

        d_layer = layer(outputs.op, inputs=[h.tensor, c.tensor],
                        weights=None,
                        const_scalars=None,
                        const_tensors=None,
                        requires_grad=self.training,
...
class SCRNN(Module):
    def __init__(self, num_units=128, context_units=64, input_size=28*28, dtype="float32", out_dtype="float32"):
        super(SCRNN, self).__init__()
        # B : [28*28, 64]
        # U: [128, 128]
        # V: [64, 128]
        # FC layer 64+28*28+128 -> 128
        self.B = Linear(input_size, context_units,
                        dtype=dtype, out_dtype=out_dtype)
        self.U = Linear(num_units, num_units,
                        dtype=out_dtype, out_dtype=out_dtype)
        self.V = Linear(context_units, num_units,
                        dtype=out_dtype, out_dtype=out_dtype)
        self.fc = Linear(num_units+context_units+input_size,
                         num_units, dtype=dtype, out_dtype=out_dtype)
