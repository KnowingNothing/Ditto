...
class SquashLayer(Module):
    ...
    def forward(self, inputs):
        ...
        output_tensor = tvm.te.compute(
            [N, C, H, W, Cap],
            lambda n, c, h, w, p: squared_norm[n]
            * inputs[n, c, h, w, p]
            / (1.0 + squared_norm[n])
            * tvm.te.sqrt(squared_norm[n]),
            name="output_tensor",
        )
...
class DigitCaps(Module):
    def __init__(
        ...
        self.W = Parameter(
            (num_channel, height, width, num_capsules, out_channels, in_channels),
            dtype=dtype,
            name="digit_cap_weight",
        )
    ...
    def forward(self, x):
        ...
        outputs = tvm.te.compute(
            ...
            lambda n, c, h, w, cap, o: tvm.te.sum(
                x[n, c, h, w, rr] * self.W[c, h, w, cap, o, rr], axis=[rr]
            ),
            name="digit_capsule",
        )
        d_layer = layer(
...
class ExpandWeightLayer(Module):
    def __init__(self, channel, height, width, cap_h, dtype="float32"):
        ...
        self.b_ij = Parameter([channel, height, width, cap_h], name="b_ij", dtype=dtype)

    def forward(self, batch):
        x = self.b_ij.tensor
        C, H, W, Cap = x.shape
        outputs = tvm.te.compute(
            [batch, C, H, W, Cap], lambda n, c, h, w, p: x[c, h, w, p], "expand"
        )
...
class SoftMaxLayer(Module):
    ...
    def forward(self, x):
        ...
        tmp = tvm.te.compute(
            [N, Cap],
            lambda n, p: tvm.te.sum(tvm.te.exp(x[n, rc, rh, rw, p]), axis=[rc, rh, rw]),
            "sum_exp",
        )
        outputs = tvm.te.compute(
            [N, C, H, W, Cap],
            lambda n, c, h, w, p: tvm.te.exp(x[n, c, h, w, p]) / tmp[n, p],
            "softmax",
        )
...
class MulCijLayer(Module):
    ...
    def forward(self, x, y):
        ...
        outputs = tvm.te.compute(
            [N, Cap1, Cap2],
            lambda n, p1, p2: tvm.te.sum(
                x[n, rc, rh, rw, p1] * y[n, rc, rh, rw, p1, p2], axis=[rc, rh, rw]
            ),
            "mul_cij_u_hat",
        )
        dy_layer = layer(
            outputs.op,
            inputs=[y.tensor, x.tensor],
            weights=None,
            const_scalars=None,
            const_tensors=None,
            requires_grad=self.training,
    ...
    def forward(self, x, y):
        ...
        outputs = tvm.te.compute(
            ...
            lambda n, c, h, w, p: tvm.te.sum(
                x[n, c, h, w, p, rp] * y[n, p, rp], axis=[rp]
            ),
            "mul_vj",
        )
        dy_layer = layer(
...
class AddLayer(Module):
    ...
    def forward(self, x, y):
        ...
        outputs = tvm.te.compute(
            [N, C, H, W, Cap1],
            lambda n, c, h, w, p1: x[n, c, h, w, p1] + y[n, c, h, w, p1],
            "add_bij",
        )
        dy_layer = layer(
            outputs.op,
...
class ActSquashLayer(Module):
    ...
    def forward(self, inputs):
        ...
        outputs = tvm.te.compute(
            [N, Cap1, Cap2],
            lambda n, p1, p2: inputs[n, p1, p2]
            * inputs[n, p1, p2]
            * inputs[n, p1, p2]
            / (1.0 + inputs[n, p1, p2] * inputs[n, p1, p2])
            * inputs[n, p1, p2],
            name="output_tensor",
        )
