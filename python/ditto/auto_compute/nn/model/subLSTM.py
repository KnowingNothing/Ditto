...
class GateLayer(Module):
    ...
    def forward(self, input_m_weight, oldh_m_weight):
        input_m_weight, oldh_m_weight = self.preprocess(input_m_weight, oldh_m_weight)
        batch_size, four_stsz = input_m_weight.shape
        outputs = tvm.te.compute(
            [batch_size, four_stsz],
            lambda i, j: tvm.te.sigmoid(input_m_weight[i, j] + oldh_m_weight[i, j]),
            name="gate_sublstm",
        )

        d_layer = layer(
            outputs.op,
            inputs=[input_m_weight.tensor, oldh_m_weight.tensor],
            weights=None,
            const_scalars=None,
            const_tensors=None,
...
class NewCLayer(Module):
    ...
    def forward(self, gates, old_c):
        ...
        outputs = tvm.te.compute(
            ...
            lambda i, j: gates[i, state_size + j] * old_c[i, j]
            + gates[i, j + 2 * state_size]
            - gates[i, j],
            name="new_c_sublstm",
        )
        ...
        d_layer = layer(
            outputs.op,
            inputs=[gates.tensor, old_c.tensor],
            weights=None,
            const_scalars=None,
            const_tensors=None,
            requires_grad=self.training,
...
class NewHLayer(Module):
    ...
    def forward(self, new_c, gates):
        ...
        d_layer = layer(
            outputs.op,
            inputs=[new_c.tensor, gates.tensor],
            weights=None,
            const_scalars=None,
            const_tensors=None,
            requires_grad=self.training,
...
class subLSTM(Module):
    def __init__(
        self, state_size=128, input_size=28 * 28, dtype="float32", out_dtype="float32"
    ):
        super(subLSTM, self).__init__()
        self.l1 = Linear(
            input_size, 4 * state_size, bias=True, dtype=dtype, out_dtype=out_dtype
        )
        self.l2 = Linear(
            state_size, 4 * state_size, bias=True, dtype=dtype, out_dtype=out_dtype
        )
        self.g1 = GateLayer()
        self.new_c = NewCLayer()
        self.new_h = NewHLayer()
        self.out = Linear(
            state_size, 10, bias=True, dtype=out_dtype, out_dtype=out_dtype
        )
