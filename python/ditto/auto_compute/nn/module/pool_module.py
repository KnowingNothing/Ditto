...
class AvgPool2d(Module):
    def __init__(self, kernel_size=2, stride=2, padding=0, layout="NCHW"):
        ...
        assert (
            isinstance(self.kernel_size, (tuple, list)) and len(self.kernel_size) == 2
        )
    ...
    def forward(self, inputs):
        ...
        if self.layout == "NCHW":
            ...
            raise NotImplementedError(f"No layout {self.layout} for avgpool.\n")
...
class GlobalAvgPool2d(Module):
    ...
    def forward(self, inputs):
        inputs = self.preprocess(inputs)
        if self.layout == "NCHW":
            outputs = global_avgpool2d_nchw(inputs.tensor, self.keep_dim)
        else:
            raise NotImplementedError(f"No layout {self.layout} for avgpool.\n")
