...
from ..functional import conv2d, conv2d_with_bias_nchw, conv2d_with_group_nchw, conv2d_capsule_nchw
...
class CapsuleConv2d(Module):
    def __init__(self, in_channel, out_channel, kernel_size,
        ...
        self.weight = Parameter(
            (out_channel, in_channel, *kernel_size, num_caps), dtype=dtype, name="conv2d_capsule_weight")
        if bias:
            self.bias = Parameter((out_channel, num_caps),
                                  dtype=out_dtype, name="conv2d_capsule_bias")
        else:
            self.bias = None
