...
def get_gemm_compute(
    ...
    res_dtype="float32",
    left_row_major=True,
    right_row_major=True,
    result_row_major=True,
):
    if left_row_major:
    ...
    if right_row_major:
        right = tvm.te.placeholder([k, n], dtype=rhs_dtype, name="right_matrix")
    else:
        right = tvm.te.placeholder([n, k], dtype=rhs_dtype, name="right_matrix")
    ...
    if left_row_major and right_row_major and result_row_major:
        result = tvm.te.compute(
            ...
            lambda i, j: tvm.te.sum(
                left[i, rk].astype(res_dtype) * right[rk, j].astype(res_dtype),
                axis=[rk],
            ),
            name="result_matrix",
        )
        ...
        result = tvm.te.compute(
            [m, n],
            lambda i, j: tvm.te.sum(
                left[i, rk].astype(res_dtype) * right[j, rk].astype(res_dtype),
                axis=[rk],
            ),
            name="result_matrix",
        )
        ...
        result = tvm.te.compute(
            [m, n],
            lambda i, j: tvm.te.sum(
                left[rk, i].astype(res_dtype) * right[rk, j].astype(res_dtype),
                axis=[rk],
            ),
            name="result_matrix",
        )
        ...
        result = tvm.te.compute(
            [m, n],
            lambda i, j: tvm.te.sum(
                left[rk, i].astype(res_dtype) * right[j, rk].astype(res_dtype),
                axis=[rk],
            ),
            name="result_matrix",
        )
...
cuda_wmma_m16_n16_k16_row_major_row_major_bf16_bf16_fp32 = visa.matrix_isa(
    ...
    get_gemm_compute(
        ...
        n=16,
        k=16,
        lhs_dtype=f"custom[{cuda_bf16}]",
        rhs_dtype=f"custom[{cuda_bf16}]",
        res_dtype="float32",
        left_row_major=True,
        right_row_major=True,
        result_row_major=True,
...
cuda_wmma_m16_n16_k8_row_major_row_major_tf32_tf32_fp32 = visa.matrix_isa(
    ...
    get_gemm_compute(
        ...
        n=16,
        k=8,
        lhs_dtype=f"custom[{cuda_tf32}]",
        rhs_dtype=f"custom[{cuda_tf32}]",
        res_dtype="float32",
        left_row_major=True,
        right_row_major=True,
        result_row_major=True,
