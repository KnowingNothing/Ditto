...
def TensorCoreGPU(name: str, sm_arch: int, param: HardwareParam):
    ...
    assert sm_arch >= 70, f"Tensor core GPU is at least is sm70, but given sm{sm_arch}.\n"
    if sm_arch < 75:
    ...
    cuda_core_isa = {
        x.name: x for x in all_scalar_isa
    }
    tensor_core_isa = {
        x.name: x for x in tc_isa
    }
    ...
    processor = hw_heteroprocessor([cuda_core, tensor_core], [
                                   register], topology, name="cuda_processor")

    shared_mem = hw_shared_mem(
        param.shared_memory_per_group_kb, scalar_patterns, name="shared_memory")

    group = hw_homogroup(
        processor, shared_mem, param.num_processors_per_group, name="streaming_multiprocessors")
