        ...
        for key, value in self.task_params.items():
            if isinstance(value, TensorizeParamChoices):
                if self.tensorize_param_key != None:
                    raise RuntimeError("Specified more than two TensorizeParamChoices!")
                self.tensorize_param_key = key
        if self.tensorize_param_key is None:
            raise RuntimeError("Did not specify TensorizeParamChoices!")
...
class DataDrivenATFTuner(object):
    ...
    def __init__(
        self, task, dev, ctx, build_timeout=10, run_timeout=10, **timeeval_params
    ):
        self.task = task
        self.dev = dev
        self.ctx = ctx
        ...
        self.builder = PopenPoolExecutor(timeout=build_timeout)
        self.runner = PopenPoolExecutor(timeout=run_timeout)
    ...
    def schedule(self, log_file="ATFLog.log"):
        ...
        log.close_file()
        if best_index == -1:
            raise RuntimeError("No applicable schedule found")
...
class GridSearchATFTuner(DataDrivenATFTuner):
    ...
    def next_batch(self, num=1):
        ...
        if num != 1:
            raise RuntimeError("Only support serial tuner for now")
        ret = self.now_index
