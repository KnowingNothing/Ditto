...
def intrin_micro_kernel_gemm2(dtype = "float32", MICRO_M = 16, MICRO_N = 16, MICRO_K = 16, prefix = ""):
    ...
    Ab = tvm.tir.decl_buffer(a.shape, a.dtype, name=prefix + "A", data_alignment=64, offset_factor=1, strides=[te.var("s1"), 1])
    Bb = tvm.tir.decl_buffer(b.shape, b.dtype, name=prefix + "B", data_alignment=64, offset_factor=1, strides=[te.var("s2"), 1])
    Cb = tvm.tir.decl_buffer(c.shape, c.dtype, name=prefix + "C", data_alignment=64, offset_factor=1, strides=[te.var("s3"), 1])
    ...
    def intrin_func(ins, outs):
        ...
        def _reduce_reset():
            ib = tvm.tir.ir_builder.create()
            ib.emit(tvm.tir.call_extern("int32", prefix + "_reset", cc.access_ptr("w"), cc.strides[0]))
            return ib.get()
...
def cpu_avx2(
    M = 16, N = 16, K = 16, dtype = "float32", prefix = ""
):
    loads = []
    compute = intrin_micro_kernel_gemm2(MICRO_M = M, MICRO_N = N, MICRO_K = K, dtype = dtype, prefix = prefix)
    store = None
    pintrin = packed_intrinsic(
        loads,
        compute,
