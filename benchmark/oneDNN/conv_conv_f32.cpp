...
long long B, C0, P0, Q0, C1, R1, S1, C2, R2, S2, pad1, pad2, stride1,
        stride2;
double PEAKGFLOPS;
...
double conv_conv(engine::kind engine_kind, int times = 100) {
    ...
    //[Create user memory]
    auto user_src_memory = memory({{conv1_src_tz}, dt::f32, tag::nChw16c}, eng);

    auto user_weights_memory
            = memory({{conv1_weights_tz}, dt::f32, tag::OIhw16i16o}, eng);
    ...
    auto conv2_user_weights_memory
            = memory({{conv2_weights_tz}, dt::f32, tag::OIhw16i16o}, eng);
    ...
    for (int j = 0; j < times; ++j) {
        ...
        flush((uint8_t *)user_src_memory.get_data_handle(), user_src_memory.get_desc().get_size());
        flush((uint8_t *)user_weights_memory.get_data_handle(), user_weights_memory.get_desc().get_size());
        flush((uint8_t *)conv1_user_bias_memory.get_data_handle(), conv1_user_bias_memory.get_desc().get_size());
        flush((uint8_t *)conv2_user_weights_memory.get_data_handle(), conv2_user_weights_memory.get_desc().get_size());
        flush((uint8_t *)conv2_user_bias_memory.get_data_handle(), conv2_user_bias_memory.get_desc().get_size());
...
//     printf("N: %d C0: %d P0 %d Q0 %d C1 %d R1 %d S1 %d C2%d R2 %d S2 %d P1 %d Q1%d P2 %d Q2%d\n", batch, C0, P0,
            //Q0, C1, R1, S1, C2, R2, S2, P1, Q1, P2, Q2);
...
int main(int argc, char **argv) {
    if (argc != 17) {
        std::cout << "Usage: ./script cpu N C0 P0 Q0 C1 R1 S1 C2 R2 S2 pad1 pad2 stride1 stride2 peak "
                     "gflops"
                  << std::endl;
        return 0;
    }
