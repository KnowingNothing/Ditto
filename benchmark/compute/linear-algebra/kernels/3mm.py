...
def _3mm(M, R, L, K, N):
    ...
    E = tvm.te.compute(
        [M, R], lambda m, r: tvm.te.sum(A[m, rk] * B[rk, r], axis=rk), name="E"
    )

    rl = tvm.te.reduce_axis([0, L], name="rl")
    F = tvm.te.compute(
        [R, N], lambda r, n: tvm.te.sum(C[r, rl] * D[rl, n], axis=rl), name="F"
    )

    rr = tvm.te.reduce_axis([0, R], name="rr")
    G = tvm.te.compute(
        [M, N], lambda m, n: tvm.te.sum(E[m, rr] + F[rr, n], axis=rr), name="G"
    )
    return [G], [A, B, C, D]
