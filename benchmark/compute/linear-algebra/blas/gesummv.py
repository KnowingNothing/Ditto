...
def gesummv(N):
    ...
    left = tvm.te.compute(
        [N], lambda n: tvm.te.sum(A[n, rn1] * x[rn1], axis=rn1), name="left"
    )

    rn2 = tvm.te.reduce_axis([0, N], name="rn2")
    right = tvm.te.compute(
        [N], lambda n: tvm.te.sum(B[n, rn2] * x[rn2], axis=rn2), name="right"
    )

    y = tvm.te.compute([N], lambda n: alpha * left[n] + beta * right[n], name="y")
