...
def symm(M, N):
    ...
    tmp = tvm.te.compute(
        [M, N], lambda m, n: tvm.te.sum(A[m, rm] * B[rm, n], axis=rm), name="tmp"
    )

    D = tvm.te.compute(
        [M, N],
        lambda m, n: beta * C[m, n] + alpha * B[m, n] * A[m, m] + alpha * tmp[m, n],
        name="D",
    )

    init = tvm.te.compute([1, M, N], lambda _, m, n: D[m, n], name="init")

    update = tvm.te.compute(
        [M, M, N],
        lambda t, m, n: tvm.tir.if_then_else(
            m < t, state[t - 1, m, n] + alpha * B[t, n] * A[t, m], state[t - 1, m, n]
        ),
        name="update",
    )
