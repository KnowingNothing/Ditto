...
def gemver(N):
    ...
    B = tvm.te.compute(
        [N, N], lambda i, j: A[i, j] + u1[i] * v1[j] + u2[i] * v2[j], name="B"
    )

    rn1 = tvm.te.reduce_axis([0, N], name="rn1")
    X = tvm.te.compute(
        [N], lambda n: tvm.te.sum(B[rn1, n] * y[rn1], axis=rn1), name="X"
    )

    XX = tvm.te.compute([N], lambda n: x[n] + beta * X[n], name="XX")

    XXX = tvm.te.compute([N], lambda n: XX[n] + z[n], name="XXX")

    rn2 = tvm.te.reduce_axis([0, N], name="rn2")
    W = tvm.te.compute(
        [N], lambda n: tvm.te.sum(B[n, rn2] * XXX[rn2], axis=rn2), name="W"
    )

    WW = tvm.te.compute([N], lambda n: w[n] + alpha * W[n], name="WW")
...
if __name__ == "__main__":
    ...
    print(
        tvm.lower(
            sch, [A, u1, u2, v1, v2, w, x, y, z, alpha, beta, WW], simple_mode=True
        )
    )
