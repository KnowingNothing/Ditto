...
def torch_conv1x1_conv3x3_cuda(A, B, C, stride1, stride2):
    with torch.autocast("cuda"):
        D = torch.nn.functional.conv2d(A, B, stride=stride1, padding=0)
        F = torch.nn.functional.conv2d(D, C, stride=stride2, padding=1)
...
def main(batch, C, H, W, K1, K2, stride1, stride2, dtype, only_once=False):
    ...
    inputs_np = [
        ...
        for y in [[batch, C, H, W], [K1, C, 1, 1], [K2, K1, 3, 3]]
    ]

    if only_once:
        inputs_torch = [torch.tensor(x).cuda() for x in inputs_np]
        output = torch_conv1x1_conv3x3_cuda(*inputs_torch, stride1, stride2)
        cost = -1
    else:
        inputs_torch = [
        ...
        for i in range(repeat):
            output = torch_conv1x1_conv3x3_cuda(*inputs_torch[i], stride1, stride2)
...
example_text = """
 example:
    python conv1x1_conv3x3_cuda.py --dtype float16 --begin 0 --num 1
"""
