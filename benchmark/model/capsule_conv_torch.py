...
class CapsuleConv2d(nn.Module):
    def __init__(
        ...
        input_capsule_size = (
            (input_capsule_size, input_capsule_size)
            if isinstance(input_capsule_size, int)
            else input_capsule_size
        )
        assert (
            isinstance(input_capsule_size, (tuple, list))
            and len(input_capsule_size) == 2
        )
        weight_capsule_size = (
            (weight_capsule_size, weight_capsule_size)
            if isinstance(weight_capsule_size, int)
            else weight_capsule_size
        )
        assert (
            isinstance(weight_capsule_size, (tuple, list))
            and len(weight_capsule_size) == 2
        )
        ...
        for i in range(self.input_capsule_size[0]):
            for j in range(self.weight_capsule_size[1]):
                ...
                for k in range(self.input_capsule_size[1]):
                    ...
                    out_ij = (
                        getattr(self, "conv2d_" + str(k) + "_" + str(j))(x_ik) + out_ij
                    )
                    out_lst.append(out_ij.unsqueeze(-1))
        out = torch.cat(out_lst, dim=-1)
        out = out.reshape(
            *out.shape[:-1], self.input_capsule_size[0], self.weight_capsule_size[1]
        )
        return out
