...
def train_perf():
    ...
    def criterion(labels, logits): return tf.nn.softmax_cross_entropy_with_logits(
        labels, logits)
    ...
    label_tensor = tf.convert_to_tensor(torch.empty(
        batch, dtype=torch.long).random_(1000).numpy())
