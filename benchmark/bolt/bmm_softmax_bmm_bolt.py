...
def relay_bmm_softmax_bmm(
    ...
    EE = relay.cast(E, dtype=in_dtype)
    C = relay.transpose(C, axes=(0, 2, 1))
    F = relay.nn.batch_matmul(EE, C, acc_dtype)
...
def main(B, M, N, K, L, in_dtype, acc_dtype, only_once):
    return verify_batch_matmul_chain(relay_bmm_softmax_bmm(B, M, N, K, L), B, M, N, K, L)


example_text = """
 example:
    python bmm_softmax_bmm_bolt.py --in_dtype float16 --acc_dtype float16 --begin 0 --num 1
"""
