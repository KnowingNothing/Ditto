...
def Conv1x1Conv3x3(
    ...
    W2 = tvm.te.placeholder([3, 3, out_channel1 // KI,  out_channel2 // NI, KI, NI], name="W2", dtype=in_dtype)
    ...
    relu = tvm.te.compute(
        [batch, out_channel1//NI, P1, Q1//MI, MI, NI],
        lambda b, lo, p, mo, mi, li: tvm.tir.if_then_else(
            cast[b, lo, p, mo, mi, li] >= tvm.tir.const(0, in_dtype),
            cast[b, lo, p, mo, mi, li],
    ...
    fract = tvm.te.compute(
        [batch, 3, 3, out_channel1//NI, P2, Q2//MI, MI, NI],
        lambda b, r, s, lo, p, mo, mi, li:
            tvm.tir.if_then_else(
                ...
                relu[b, lo, p + r - 1, (mo * MI + mi + s - 1)//MI, (mo * MI + mi + s - 1)%MI, li],
                tvm.tir.const(0, in_dtype)
            ),
        name="fract"
    )
    ...
    W2_shared = tvm.te.compute(
        [3, 3, out_channel1 // KI,  out_channel2 // NI, KI, NI],
        lambda r, s, lo, no, li, ni: W2[r, s, lo, no, li, ni],
        name="W2_shared",
    )
    ...
    Conv2_frag = tvm.te.compute(
        ...
        lambda b, no, p, mo, mi, ni: tvm.te.sum(
            fract[b, rr, rs, rlo, p, mo, mi, rli].astype(acc_dtype)
            * W2_shared[rr, rs, rlo, no, rli, ni].astype(acc_dtype),
            axis=[rlo, rr, rs, rli],
        ),
        name="Conv2_frag",
    )
...
def main(
    ...
    fract, C_ext = E_frag.op.input_tensors
    relu = fract.op.input_tensors[0]
    cast = relu.op.input_tensors[0]
    D_frag = cast.op.input_tensors[0]
    A_shared, B_shared = D_frag.op.input_tensors

    b, no, p, mo, mi, ni = E_frag.op.axis
    rlo, rr, rs, rli = E_frag.op.reduce_axis
    fuse_choice = at.fusion_choice(D_frag.op, E_frag.op, [b, no, p, mo, rlo, mi, ni, rr, rs, rli], 4)
...
example_text = """
 example:
    python conv1x1_conv3x3_cuda.py --in_dtype float16 --acc_dtype float16 --begin 0 --num 1
"""
