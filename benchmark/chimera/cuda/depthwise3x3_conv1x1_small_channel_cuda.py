...
def cal_ai(batch, C, H, W, K, stride1, stride2, factor):
    return (batch*H//stride1*W//stride1*uround(factor,NI)/(batch*H//stride1*W//stride1+uround(factor,NI)),
            batch*H//stride1//stride2*W//stride1//stride2*K/(batch*H//stride1//stride2*W//stride1//stride2+K))
...
if __name__ == "__main__":
    ...
    if args.ai:
        ...
        print("batch,C,H,W,K,stride1,stride2,factor,in_dtype,acc_dtype,sm,cost")
        for cc in costs:
            print(
                f"{args.batch},"
                + ",".join(map(str, cc[0]))
