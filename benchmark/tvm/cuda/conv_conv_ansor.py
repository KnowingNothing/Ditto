...
def conv_conv(batch, C, H, W, K1, K2, stride1, stride2, k1, k2, in_dtype, acc_dtype):
    ...
    conv1 = topi.nn.conv2d_nchw(Img, W1, stride1, k1 // 2, out_dtype=acc_dtype, dilation=1)
    cast1 = tvm.te.compute(
        conv1.shape, lambda *args: conv1(*args).astype(in_dtype), name="cast"
    )
    conv2 = topi.nn.conv2d_nchw(cast1, W2, stride2, k2 // 2, out_dtype=acc_dtype, dilation=1)
...
def main(
    ...
    log_file = f"conv_conv_{batch}-{C}-{H}-{W}-{K1}-{K2}-{stride1}-{stride2}-{k1}-{k2}-{in_dtype}-{acc_dtype}.json"
...
example_text = """
 example:
    python conv_conv_ansor.py --in_dtype float16 --acc_dtype float32 --begin 0 --num 1 --sm 80
"""
