...
def conv_relu_conv(
    ...
    Weight2 = tvm.te.placeholder(
        [C2, C1, R2, S2], dtype=in_dtype, name="Weight2")
    ...
    Conv1_rfact = tvm.te.compute([N, C1, P1, Q1], lambda n, c, p, q:
                                 Conv1[n // mk1.N, n % mk1.N, c // mk1.K, c %
                                       mk1.K, p // mk1.H, p % mk1.H, q // mk1.W, q % mk1.W],
    ...
    Pad2 = tvm.te.compute([N, C1, P1 + 2 * padding2, Q1 + 2 * padding2],
                          lambda n, c, p, q: tvm.tir.if_then_else(tvm.tir.all(
                              p >= padding2, p < P0+padding2, q >= padding2, q < Q0 + padding2), Conv1_relu[n, c, p-padding2, q-padding2], tvm.tir.const(0, in_dtype)),
                          name="pad2"
                          )
    ...
    Conv2TensorizeAxes = [Conv2_fact.op.axis[1], Conv2_fact.op.axis[3],
                          Conv2_fact.op.axis[5], Conv2_fact.op.axis[7], ci2, r2, s2]
    ...
    return [Img, Weight1, Weight2], [Conv2], Conv1TensorizeAxes + Conv2TensorizeAxes, Conv1TensorizeAxes, Conv2TensorizeAxes
...
def BatchGemmSoftmaxGemm(
    ...
    relu = tvm.te.compute(
        [batch, M // MI, L // NI1, MI, NI1],
        lambda b, mo, lo, mi, li: tvm.tir.if_then_else(D_frag[b, mo, lo, mi, li] > 0, D_frag[b, mo, lo, mi, li], 0).astype(
            in_dtype
        ),
        name="relu",
    )
    ...
    C_ext = tvm.te.compute(
        ...
        lambda b, lo, no, li, ni: 
            C[b, lo * KI2 + li, no * NI2 + ni],
        name="C_ext",
    )
    ...
    F = tvm.te.compute(
        ...
        lambda b, m, n: E_frag[b, m // MI, n // NI2, m %
                               MI, n % NI2].astype(in_dtype),
        name="F",
    )

    return [A, B, C], [F], [D_frag.op.axis[-2], D_frag.op.axis[-1], rki, E_frag.op.axis[-1], E_frag.op.axis[-2], rli], \
        [D_frag.op.axis[-2], D_frag.op.axis[-1],
            rki], [E_frag.op.axis[-2], E_frag.op.axis[-1], rli]
...
def test_fusion_choice_handwritten():
    ...
    fuse_choice = at.fusion_choice(
        D_frag.op, E_frag.op, [b, m, n, rlo, mi, ni, rli], 3)
    print(fuse_choice)
...
def test_tensorize_cpu():
    ...
    first_packed = at.cpu_avx2(
            M=MI, N=NI1, K=KI1, dtype="float32", prefix="gemm1")
   ...
   # choice = first_match_info_choices[0]
    ...
    second_packed = at.cpu_avx2(
            M=MI, N=NI2, K=KI2, dtype="float32", prefix="gemm2")
...
def test_tensorize_cpu_run(instructionSet, parallelism=1):
   ...
   # choice = first_match_info_choices[0]
    ...
    cost = evaluator(*inputs_tvm, *outputs_tvm).mean * 1e3
    print(f"Our code uses {cost} ms")
    print(f"gflops:{workload / 1e6 / cost}, peak:{parallelism*16*2.2}, ratio: {workload / 1e6 / cost/(parallelism*16*2.2)}")


def test_tensorize_cpu_run_conv(instructionSet, parallelism=1, config={}):
    cacheSizes = [1024, 32 * 1024, 256*1024,
                  35840*1024]
    bandwidth = [293.72, 81.72, 38.54,  13.14]
    tensorWeight = [1.0, 2.0]
    ...
    t0 = time.time()
    fuse_choice = at.build_fusion_choice(
        sfs, hw_param=CPU, dtype="float32",
        simple_mode=-1)
    fusionResult = fuse_choice.fusion_result
    workload = fusionResult.workload
    print(fusionResult)
...
def test_model_validation(batch, M, N, K, L, instructionSet="avx2", prefix="", config={}):
    ...
    fusionContext = at.build_fusion_context(
        sfs, layer, state, code,
        data_path, hw_param=CPU, dtype="float32", searchType=searchType, mode="survey")
    ...
    for iter in range(fusionContext.size):
        ...
        cmd1 = f'python ./runCommand.py python ./profileScript.py {filename} {modulename}'
        ...
        metrics.append({'time': cost, 'computation': computation,
                       'toPeak': ratioToPeak, 'predCost': predCost, 'occupancy': occupancy})
        if mode == 'best':
            break
        with open(res_path, 'a') as f:
            f.write(
                f'{iter} {ratioToPeak} {1 / ratioToPeak}\n')
    ...
    for shape in GEMMData:
        ...
        try:
            _, _, data_path, res_path, metrics = test_model_validation(
                batch=B, M=M, N=K, K=K, L=L, instructionSet="avx2", prefix=prefix, config=config)
            valid += 1
        except:
...
if __name__ == "__main__":
    ...
    test_model_validation(1,2052,128,128,32,"avx2",  config = {"searchType": "stochastic", "mode": "best", 'parallelism': 1})
    cacheSizes = [32*16, 32 * 1024, 256*1024,
                  35840*1024] #, 131749016]
