...
def test2():
    ...
    layer = ac.layer(F.op, inputs=[A], weights=[B, C, D], const_scalars=[alpha, beta])
    ...
    input_A = ac.layer_tensor(
        layer_state[A].op.output(0).shape, name="real_A", dtype="float32"
    )
...
def test3():
    ...
    print(tvm.lower(sch, all_tensors, simple_mode=True))
    new_func = tvm.build(sch, all_tensors, "llvm")
    new_C_np = np.random.uniform(
        -10, 10, [int(x) for x in layer_state[C].op.output(0).shape]
    ).astype("float32")
