...
namespace std {
template <>
struct hash<::ditto::hybrid::TensorDimKey> {
  std::size_t operator()(const ::ditto::hybrid::TensorDimKey& k) const {
...
AttachPath CreateAttachPath(HybridSchedule sch) {
  ...
  for (Stage stage : sch->stages) {
    ...
    for (Stage s = stage; s.defined();) {
      ...
      ICHECK(start_attach) << "Invalid HybridSchedule: cannot find attach point " << attach_ivar
                           << " in the hybrid_schedule of " << s->op;
    }
...
TVM_REGISTER_GLOBAL("ditto.CreateReadGraph").set_body_typed(CreateReadGraph);

TVM_REGISTER_GLOBAL("ditto.PostDFSOrder")
    .set_body_typed([](const Array<Operation>& roots, const ReadGraph& g) {
      return PostDFSOrder(roots, g);
    });

TVM_REGISTER_GLOBAL("ditto.CreateAttachPath").set_body_typed(CreateAttachPath);

TVM_REGISTER_GLOBAL("ditto.ScanGetBody").set_body_typed(ScanGetBody);

TVM_REGISTER_GLOBAL("ditto.ScanFixPointAnalysis").set_body_typed(ScanFixPointAnalysis);
