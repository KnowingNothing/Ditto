...
Stmt MakePipeline(const HybridStage& s, const std::unordered_map<IterVar, Range>& dom_map, Stmt consumer,
                  bool debug_keep_trivial_loop) {
  Stmt producer = BuildProvide(s, dom_map, debug_keep_trivial_loop);
  if (s->double_buffer) {
  ...
  return BuildRealize(s, dom_map, pipeline, s->scope);
}

// inject the operator's realization on the stmt.
class InjectAttach : public StmtMutator {
 public:
  InjectAttach(const HybridStage& stage, const HybridStage& attach_spec,
 ...
 private:
  // The stage.
  const HybridStage& stage_;
  // The attach spec, may not contain op.
  const HybridStage& attach_spec_;
  // domain map
...
class InjectScanStep : public StmtMutator {
 public:
  InjectScanStep(const HybridStage& stage, const Operation& scan_op,
...
class SchedulePostProc : public StmtExprMutator {
 public:
  ...
  void Init(const HybridSchedule& sch) {
...
Stmt ScheduleOps(HybridSchedule sch, Map<IterVar, Range> dom_map_, bool debug_keep_trivial_loop) {
  Stmt body = Stmt();
  ...
  for (HybridStage g : sch->groups) {
    ICHECK(!g->op.defined());
    ICHECK_EQ(g->leaf_iter_vars.size(), 0U);
  }
  // reverse the post DFS order.
  for (size_t i = sch->stages.size(); i != 0; --i) {
    HybridStage s = sch->stages[i - 1];
    ...
    HybridStage attach_spec = s.GetAttachSpec();
...
TVM_REGISTER_GLOBAL("ditto.HybridScheduleOps").set_body([](TVMArgs args, TVMRetValue* ret) {
  if (args.size() == 2)
