...
struct GraphContext {
  ...
  /*! \brief map from op to hybrid_stage */
  std::unordered_map<const Object*, HybridStage> op2stage_;
};
...
StorageScope InferStorageScope(const HybridStage& stage, const GraphContext& ctx) {
...
void InferRootBound(const HybridStage& stage, const GraphContext& ctx,
  ...
  for (const Operation& op : consumers) {
    ...
    const HybridStage& op_stage = ctx.op2stage_.at(op.get());
    // Consumer nest
...
Map<IterVar, Range> InferBound(const HybridSchedule& sch) {
  ...
  for (size_t i = sch->stages.size(); i != 0; --i) {
    const HybridStage& stage = sch->stages[i - 1];
...
TVM_REGISTER_GLOBAL("ditto.InferBound").set_body_typed(InferBound);
