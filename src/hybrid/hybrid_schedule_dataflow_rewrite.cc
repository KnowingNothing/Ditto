...
#include "../../3rdparty/tvm/src/te/schedule/operation_inline.h"
...
Tensor HybridSchedule::cache_read(const Tensor& tensor, const std::string& scope,
  ...
  // when a hybrid_schedule has multiple cache_read on the same tensor,
...
void PrepareAxisMapping(HybridStage orig_stage, OpType* op, std::unordered_set<IterVar>* p_red_axis,
  ...
  hybrid::PassDownDomain(orig_stage, &dom_map, &analyzer, true);
  {
    // The source->cache
...
Array<Tensor> CacheWriteWithReLayout(HybridSchedule sch, const Array<Tensor>& tensor_array,
  ...
  Array<PrimExpr> args;
  {
    ...
    hybrid::PassDownIndex(orig_stage, dom_map, &value_map, true);
    for (IterVar iv : orig_stage->leaf_iter_vars) {
      if (red_axis.count(iv)) continue;
...
Array<Tensor> CacheWriteWithReLayoutTensor(HybridSchedule sch, const Array<Tensor>& tensor_array,
                                           const std::string& scope) {
  size_t tensor_size = tensor_array.size();
  sch->InvalidateCache();
  Tensor tensor = tensor_array[0];
...
Array<Tensor> HybridSchedule::cache_write(const Array<Tensor>& tensor_array, const std::string& scope) {
  (*this)->InvalidateCache();
...
void InjectInline(HybridScheduleNode* sch, bool feature_extraction_mode) {
  sch->InvalidateCache();
...
void LegalizeInvalidAttach(HybridScheduleNode* sch) {
...
HybridSchedule HybridSchedule::normalize() {
  HybridSchedule sn = copy();
...
HybridSchedule HybridSchedule::normalize_for_feature_extraction() {
  HybridSchedule sn = copy();
...
Array<Tensor> HybridSchedule::rfactor(const Tensor& tensor, const IterVar& axis, int factor_axis) {
  (*this)->InvalidateCache();
  using tir::ReduceNode;
  ...
  touch_map[axis] = 1;
  hybrid::PassUpBitMaskOr(reduce_stage, &touch_map, true);
  hybrid::PassDownBitMaskOr(reduce_stage, &touch_map, true);
  // skip reduction iteration.
  std::unordered_set<IterVar> skip_bound_check;
  ...
  hybrid::PassDownDomain(reduce_stage, &dom_map, &analyzer, true);
  ...
  hybrid::PassUpIndex(reduce_stage, dom_map, &value_map, true);
  std::vector<PrimExpr> predicates =
