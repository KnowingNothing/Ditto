...
HardwareParam::HardwareParam(
    ...
    std::vector<double> tensorWeight, std::vector<double> cacheSizes,
    std::vector<double> bandwidth, std::vector<double> coresPerCacheLevel,
    std::string platform) {
...
TVM_REGISTER_GLOBAL("ditto.hardware.HardwareParam")
    .set_body_typed(
        [](double register_per_processor_kb, double shared_memory_per_group_kb,
           ...
           Array<FloatImm> coresPerCacheLevel = {}, String platform = "NVGPU") {
          ...
          return HardwareParam(
              ...
              cacheSizes_, bandwidth_, coresPerCacheLevel_,
              std::string(platform));
        });
