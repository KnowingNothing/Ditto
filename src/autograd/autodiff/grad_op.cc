...
#define NOT_IMPLEMENTED                                                        \
  {                                                                            \
    LOG(FATAL) << "Grad of this expr is not implemented: "                     \
               << GetRef<PrimExpr>(op);                                        \
    throw;                                                                     \
  }
...
private:
  ...
  // bool first_met_;
public:
  explicit GradOp(NameGenerator &generator, SubstituteContext &context,
                  const Tensor &input, const Tensor &doutput,
                  Array<PrimExpr> &call_args, Array<PrimExpr> compute_args)
      : generator_(generator), context_(context), input_(input),
        doutput_(doutput), call_args_(call_args), compute_args_(compute_args) {
    ...
    if (input_.get() && tensor == input_) {
      ...
      for (int i = 0; i < cols; ++i) {
        ...
        if (bindings.count(context_.index_names[i]) > 0) {
          ...
          bindings[context_.index_names[i]] =
              std::vector<PrimExpr>({ana.Simplify(bind_val)});
        }
      }
      ...
      for (auto it : sub_set) {
        ...
        if (it.first == "") {
          ...
          context_.range_map[new_name] =
              context_.range_map[it.var_name].floor_div(it.factor);
        } else {
        ...
        if (it.second == "") {
          ...
          CHECK(context_.range_map.count(it.var_name) != 0)
              << "We should know var: " << it.var_name << ".\n";
          context_.range_map[new_name] =
              context_.range_map[it.var_name].floor_mod(it.factor);
        } else {
      ...
      for (auto kv : results) {
        ...
        // if (as_var != nullptr && relaxes.count(as_var->name_hint) != 0) {
        //   // we should know the lhs
        //   CHECK(context_.range_map.count(kv.first) != 0) << "Internal error:
        //   unknown var: "
        //                                                  << kv.first <<
        //                                                  ".\n";
        //   context_.range_map[as_var->name_hint] =
        //   context_.range_map[kv.first];
        // }
...
#ifdef DEBUG_AUTODIFF
          std::cout << kkv.first << ": [" << kkv.second.left << ", "
                    << kkv.second.right << ")\n";
#endif
          if (context_.range_map.count(kkv.first) == 0 ||
              context_.range_map[kkv.first].range_type() !=
                  ExtRangeType::LCRC) {
        ...
        if (kv.second.as<VarNode>() == nullptr) {
          CHECK(context_.range_map[kv.first].range_type() ==
                ExtRangeType::LCRC);
      ...
      // prepare source
      // result_expr = Call(op->dtype, doutput_->op->name, call_args_,
      // CallNode::Halide,
      ...
      for (std::string it : relaxes) {
        ...
        pos_vmap.Set(context_.var_map[it],
                     Add(context_.var_map[it], range.left));
        IterVar iv =
            IterVar(Range(0, pos_ext), context_.var_map[it], kCommReduce);
      ...
      if ((int)new_axis.size() == 0) {
        result_expr = ana.Simplify(Select(result_condition, result_expr,
                                          make_const(result_expr.dtype(), 0)));
        return result_expr;
      }
      // form reduce
  ...
  PrimExpr VisitExpr_(const CallNode *op) {
    PrimExpr expr = GetRef<PrimExpr>(op);
    std::unordered_set<RelayExpr, ObjectPtrHash, ObjectPtrEqual>
        piecewise_const = {Op::Get("tir.floor"), Op::Get("tir.ceil"),
                           Op::Get("tir.trunc"), Op::Get("tir.round")};
    if (op->op.same_as(Op::Get("tir.exp"))) {
      ...
      vmap_scope_.push_back(vmap);
      return Mul(new_arg0,
                 Sub(FloatImm(new_expr.dtype(), 1.0), Mul(new_expr, new_expr)));
  ...
  PrimExpr VisitExpr_(const DivNode *op) {
    ...
    vmap_scope_.pop_back();
    vmap_scope_.push_back(vmap);
    return Div(Sub(Mul(new_a, sub_b), Mul(sub_a, new_b)), Mul(sub_b, sub_b));
  }
  ...
  PrimExpr VisitExpr_(const ModNode *op) NOT_IMPLEMENTED
    ...
    vmap_scope_.pop_back();
    vmap_scope_.push_back(vmap);
    return FloorDiv(Sub(Mul(new_a, sub_b), Mul(sub_a, new_b)),
                    Mul(sub_b, sub_b));
  }
...
protected:
  ...
  PrimExpr VisitExpr_(const AddNode *op) override {
    ...
    if (a_as_red != nullptr && b_as_red != nullptr) {
      ...
      if (a_as_red->axis.size() == b_as_red->axis.size() &&
        ...
        for (size_t i = 0; i < a_as_red->axis.size(); ++i) {
          ...
          for (size_t j = 0; j < b_as_red->axis.size(); ++j) {
            if (visit.count(j) != 0) {
              continue;
            } else if (cee(a_as_red->axis[i]->dom->min,
                           b_as_red->axis[j]->dom->min) &&
                       cee(a_as_red->axis[i]->dom->extent,
                           b_as_red->axis[j]->dom->extent)) {
              visit.insert(j);
              pos_map.push_back(j);
        ...
        if (equal) {
          ...
          for (auto expr : a_as_red->source) {
            new_source.push_back(
                Substitute(Add(expr, b_as_red->source[i]), vmap));
            ++i;
          }
      ...
      // TODO(size): what's the difference of init?
      return Reduce(b_as_red->combiner, new_source, b_as_red->axis,
                    b_as_red->condition, b_as_red->value_index, b_as_red->init);
    }
    arith::Analyzer ana;
    ...
    if (a_as_red != nullptr && b_as_red != nullptr) {
      ...
      if (a_as_red->axis.size() == b_as_red->axis.size() &&
        ...
        if (equal) {
          ...
          size_t i = 0;
          for (auto iv : a_as_red->axis) {
            // // std::cout << "check temp b iv=" <<
            // b_as_red->axis[pos_map[i]]->var << ", a iv=" << iv->var << "\n";
            vmap.Set(b_as_red->axis[pos_map[i]]->var, iv->var);
            ++i;
          }
          Array<PrimExpr> new_source;
          ...
          for (auto expr : a_as_red->source) {
            new_source.push_back(
                Substitute(Sub(expr, b_as_red->source[i]), vmap));
            ++i;
          }
          // TODO(size): what's the difference of init?
          return Reduce(a_as_red->combiner, new_source, a_as_red->axis,
  ...
  PrimExpr VisitExpr_(const CastNode *op) override {
    ...
    if (as_red != nullptr) {
      ...
      // TODO(size): what's the difference of init?
      return Reduce(as_red->combiner, new_source, as_red->axis,
                    as_red->condition, as_red->value_index, as_red->init);
    }
    arith::Analyzer ana;
...
public:
  ...
  FormCompute(NameGenerator &generator, const std::string &tensor_name,
      ...
      : generator_(generator), tensor_name_key_(tensor_name), shape_(shape),
        sub_vars_(sub_vars), tag_(tag), count_tag_(0) {}
...
protected:
  ...
  void VisitExpr_(const SizeVarNode *op) override UNEXPECTED
    ...
    auto func = [=](const Array<Var> &input_indices) {
      ...
      // return Substitute(
      //     Call(op->dtype, op->name, op->args, op->call_type, op->func,
      //     op->value_index), vmap);
  ...
  void visit_binary_op(const TNode *op) {
    ...
    if (a_red != nullptr && b_red != nullptr) {
      auto func = [=](const Array<Var> &input_indices) {
        ...
        //                                  CallNode::CallType::Halide, ta->op,
        //                                  ta->value_index);
        // PrimExpr b_body = Call(tb->dtype, tb->op->name, call_args,
        //                                  CallNode::CallType::Halide, tb->op,
        //                                  tb->value_index);
  ...
  void VisitExpr_(const FloorDivNode *op) override {
    visit_binary_op<FloorDiv>(op);
  }

  void VisitExpr_(const FloorModNode *op) override {
    visit_binary_op<FloorMod>(op);
  }
  ...
  void VisitExpr_(const EQNode *op) override UNEXPECTED
    ...
    auto func = [=](const Array<Var> &input_indices) {
      ...
      // TODO(size): what's the difference of init?
      return Substitute(Reduce(op->combiner, op->source, op->axis,
                               op->condition, op->value_index, op->init),
                        vmap);
    };
  ...
  void VisitExpr_(const CastNode *op) override {
    ...
    if (as_red != nullptr) {
      auto func = [=](const Array<Var> &input_indices) {
        ...
        // PrimExpr body = Call(t->dtype, t->op->name, call_args,
  ...
  void VisitExpr_(const NotNode *op) override UNEXPECTED
    ...
    if (t_as_red != nullptr && f_as_red != nullptr) {
      auto func = [=](const Array<Var> &input_indices) {
        ...
        //                    CallNode::CallType::Halide, true_tensor->op,
        //                    true_tensor->value_index);
        // PrimExpr false_body =
        //     Call(false_tensor->dtype, false_tensor->op->name, call_args,
        //                    CallNode::CallType::Halide, false_tensor->op,
...
// PrimExpr change_iter_var_names(const PrimExpr &body, const Array<IterVar>
// &ivs, const Array<IterVar> &nivs) {
//   Map<Var, PrimExpr> vmap;
...
PrimExpr ensure_unique_var(const ComputeOpNode *op, SubstituteContext &context,
  ...
  for (auto iter_var : op->axis) {
    ...
    context.range_map[new_name] =
        ExtRange(iter_var->dom->min,
                 Add(iter_var->dom->min, iter_var->dom->extent), false, false);
  ...
  if (const ReduceNode *red = body.as<ReduceNode>()) {
    ...
    for (auto iv : red->axis) {
      ...
      context.range_map[new_name] = ExtRange(
          iv->dom->min, Add(iv->dom->min, iv->dom->extent), false, false);
      vmap.Set(iv->var, new_var);
    }
    // modify the sources
    // Array<PrimExpr> new_source;
    // for (auto src : red->source) {
    //   new_source.push_back(change_iter_var_names(src, red->axis,
    //   new_iter_vars));
    // }

    // PrimExpr new_condition = change_iter_var_names(red->condition, red->axis,
    // new_iter_vars);
...
Tensor grad_op(const Tensor &input, const Tensor &output,
               const Tensor &doutput) {
  const ComputeOpNode *op = output->op.as<ComputeOpNode>();
  CHECK(op) << "Derivative of this operation is not implemented: "
            << output->op;
  ...
  CHECK(is_input_tensor)
      << "grad_op is called on a pair of tensors such that the output "
  ...
  Array<PrimExpr> call_args;
  PrimExpr body =
      ensure_unique_var(op, context, generator, call_args, output->value_index);
  // PrimExpr new_body;
  ...
  if (as_red != nullptr) {
    ...
    for (auto expr : as_red->source) {
      ...
      GradOp helper(generator, new_context, input, doutput, call_args,
                    compute_args);
  ...
  FormCompute former(generator, generator.unique_name("_tensor"), shape,
                     sub_vars, new_tag);
...
TVM_REGISTER_GLOBAL("ditto.autograd.grad_op")
