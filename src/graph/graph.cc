...
bool IsConstInt(const PrimExpr& expr) { return (expr.as<tir::IntImmNode>() != nullptr); }
...
std::vector<LayerTensor> Layer::ProduceOutputs(std::vector<LayerTensor> layer_inputs) {
  auto self = (*this);
...
void TensorStateNode::SplitDim(int ordinal, tir::IterVar outer, tir::IterVar inner) {
  auto self = this;
...
void OpStateNode::SplitSpatial(tir::IterVar iv, PrimExpr factor, tir::IterVar* p_outer,
  ...
  tir::IterVar outer(Range(tir::make_const(nparts->dtype, 0), const_nparts),
                     tir::Var(iv->var->name_hint + ".o", iv->var->dtype), iv->iter_type,
                     iv->thread_tag);
  tir::IterVar inner(Range(tir::make_const(nparts->dtype, 0), const_factor),
                     tir::Var(iv->var->name_hint + ".i", iv->var->dtype), iv->iter_type,
                     iv->thread_tag);
...
void OpStateNode::SplitReduce(tir::IterVar iv, PrimExpr factor, tir::IterVar* p_outer,
                              tir::IterVar* p_inner, int* ordinal) {
  auto self = this;
  // check if the required axis exists
  bool exist{false};
...
void OpStateNode::PropagateSplit(te::Operation op, int ordinal, tir::IterVar outer,
                                 tir::IterVar inner) {
...
void BlockStateNode::Split(Layer layer, te::Operation op, tir::IterVar iv, PrimExpr factor,
                           tir::IterVar* p_outer, tir::IterVar* p_inner, int* ordinal) {
...
TVM_REGISTER_GLOBAL("ditto.Graph").set_body_typed([](std::string name, Array<Block> block_list) {
