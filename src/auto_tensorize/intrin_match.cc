...
Array<IterVar> IntrinMatcher::_extract_axes_from_op(const ComputeOpNode *op,
                                                    bool include_reduce) {
  Array<IterVar> axes;
  for (IterVar axis : op->axis)
    axes.push_back(axis);
  if (include_reduce) {
    for (IterVar axis : op->reduce_axis)
      axes.push_back(axis);
  }
...
bool IntrinMatcher::_match(Tensor target, Operation capsule,
                           Map<IterVar, Range> target_bounds,
                           Map<IterVar, Range> intrin_bounds, bool innermost,
                           bool samerange) {
  size_t ivd = 0;
  ...
  possible_index_mappings =
      expr_matcher.match(target_expr, intrin_expr, target_axes, intrin_axes,
...
void CapsuleExprMatcher::extract_indices(
    PrimExpr target, PrimExpr intrin, Array<Array<PrimExpr>> &target_indices,
...
Array<IterVarMap> CapsuleExprMatcher::match(PrimExpr target, PrimExpr intrin,
  ...
  possible_index_mappings =
      index_matcher.match(target_indices, intrin_indices, target_axes,
...
Array<IterVarMap> enumerate_mappings(Array<IterVar> target_axes,
                                     Array<IterVar> intrin_axes) {
  ...
  auto comp = [](const IterVar &x, const IterVar &y) {
    return x.get() < y.get();
  };
...
bool IndexExprMatcher::_match_index(Array<PrimExpr> target_idx,
  ...
  for (PrimExpr i : target_idx) {
    if (!is_const_int(i)) {
      ...
      std::cout << "Warning: found a non-zero constant in target_idx"
                << std::endl;
...
Array<Array<PrimExpr>> IndexExprMatcher::_rewrite_indices(
    Array<Array<PrimExpr>> indices, IterVarMap itervar_map,
  ...
  auto simplify = [](const PrimExpr &x) {
    return arith::Analyzer().Simplify(x);
  };
...
Array<IterVarMap> IndexExprMatcher::match(Array<Array<PrimExpr>> target_indices,
  ...
  Array<IterVarMap> all_itervar_mappings =
      enumerate_mappings(target_axes, intrin_axes);

  for (IterVarMap itervar_map : all_itervar_mappings) {
    auto modified_target_indices = _rewrite_indices(
        target_indices, itervar_map, target_bounds, intrin_bounds);
...
TVM_REGISTER_GLOBAL("ditto.auto_tensorize.MatchIntrinsic")
    .set_body_typed([](Tensor target, Operation capsule, bool innermost,
                       bool samerange) {
