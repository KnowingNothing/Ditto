...
namespace ditto
{
    ...
    namespace auto_tensorize
    {
        ...
        IterVar::IterVar(int idx_, FACTOR ext_, IV_Type iv_type_, tvm::tir::Var name_, tvm::tir::Var originVar)
        {
        ...
        std::vector<int> AccessFunctionNode::getFootprint(Map<tir::Var, IntImm> bounds){
            int idx = 0;
            ...
            for (auto access_index: access_indices){
                ...
                Map<tir::Var, Range> var_range_map = utils::InferRange(vars_to_infer, ori_ranges);
                int fp = 1;
                for(auto kv: var_range_map)
                    fp *= kv.second->min.as<IntImmNode>()->value + kv.second->extent.as<IntImmNode>()->value;
                ret.push_back(fp);
            }
            return ret;
        }
        ...
        IterGraph::IterGraph(Array<IterVar> firstOpIters,
        ...
        void IterGraphNode::setFirstOpTiling(Array<IntImm> factors_)
        {
            ...
            for(auto i: factors_) factors.push_back(i->value);
            CHECK(factors.size() == _firstOpIters.size()) << "the length of input factors does not match first Ops private iters";
            ...
            for (size_t i = 0; i < factors.size(); i++)
            {
                ...
                inner_ = IterVar(parent->index, factors[findPos(parent)], parent->iv_type,
        ...
        void IterGraphNode::setSecondOpTiling(Array<IntImm> factors_)
        {
            std::vector<FACTOR> factors;
            for(auto i: factors_) factors.push_back(i->value);
            CHECK(factors.size() == _secondOpIters.size()) << "the length of input factors does not match second Ops private iters";
            auto findPos = [&](IterVar iv){
                int idx = 0;
        ...
        void IterGraphNode::setFirstOpPermute(Array<IntImm> _permutation)
        {
            ...
            for(auto i: _permutation) permutation.push_back(i->value);
            std::vector<size_t> permutation_ = permutation;
            std::sort(permutation_.begin(), permutation_.end());
            CHECK(permutation_.size() == _firstOpIters.size()) << "setPermute takes Sigma([len(firstOpIters)]) as input.";
            for (size_t i = 0; i < permutation_.size(); i++)
                CHECK(permutation_[i] == i) << "setPermute takes Sigma([len(firstOpIters)]) as input.";
        ...
        void IterGraphNode::setSecondOpPermute(Array<IntImm> _permutation)
        {
            ...
            std::vector<size_t> permutation_ = permutation;
            std::sort(permutation_.begin(), permutation_.end());
            CHECK(permutation_.size() == _secondOpIters.size()) << "setPermute takes Sigma([len(secondOpIters)]) as input.";
            for (size_t i = 0; i < permutation_.size(); i++)
                CHECK(permutation_[i] == i) << "setPermute takes Sigma([len(secondOpIters)]) as input.";
        ...
        {
            CHECK(attach_pos <= _secondOpIters.size() && attach_pos >= 0) << "attach pos should be in range [0, len(secondOpIters)]";
            attachPos = attach_pos;
        }
        
        void IterGraphNode::setSchedule (FusionItem fusionItem){
        ...
        FusionSpace IterGraphNode::getSearchSpace(){
            ...
            return FusionSpace(PermuteSpace(_firstOpIters.size()), PermuteSpace(_secondOpIters.size()),\
                            TilingSpace(getExtents(_firstOpIters)), TilingSpace(getExtents(_secondOpIters)),\
                            AttachSpace(_secondOpIters.size())
                                );
        }

        void IterGraphNode::synchronize()
        {
            // 0. check if both ops are splited, clear shared bit
            CHECK(firstOpIters.size() > _firstOpIters.size()) << "split op1 before synchronize";
            CHECK(secondOpIters.size() > _secondOpIters.size()) << "split op2 before synchronize";
            for(auto iv: firstOpIters) iv->shared = false;
            for(auto iv: secondOpIters) iv->shared = false;
            ...
            // 2. find the shared axis & common axis and replace the axis in shared relation.
            auto findSplitPos = [&](IterVar iv)
            {
                for (auto i : splitRelations)
                    if (i->outer == iv)
                        return i;
                CHECK(false) << iv << " not found";
            };
            ...
            for (auto split : splitRelations){
                ...
                CHECK(split->parent->ext >= 0 && split->inner->ext >= 0) << "the split parent or inner's extent hasn't set";
                split->outer->ext = (split->parent->ext + split->inner->ext - 1) / split->inner->ext;
            }
        }
        ...
        Map<tir::Var, IntImm> IterGraphNode::inferBound() const{
            ...
            for (auto split: splitRelations){
                if(is_common(split->outer))
                    bounds.Set(split->inner->originVar, IntImm(DataType::Int(32), split->inner->ext));
                else
                    bounds.Set(split->inner->originVar, IntImm(DataType::Int(32), split->parent->ext));
            }
            return bounds;
        }
        ...
        int IterGraphNode::getNumOfBlocks() const{
            ...
            for(auto iv: commonIters){
                CHECK(iv->ext >= 0) << "the extent for outerLoops is unset. Have you called synchronize?";
                ret *= iv->ext;
            }
            return ret;
        }
        ...
        int IterGraphNode::getFirstOpFp() const{
            Map<tir::Var, IntImm> bounds = inferBound();
        ...
        int IterGraphNode::getFirstOpBlockSize() const{
            Map<tir::Var, IntImm> bounds = inferBound();
            int fp = 0;
            for(auto acf: firstOpReadAccessFuncs)
                for(auto fp_: acf->getFootprint(bounds))
        ...
        int IterGraphNode::getSecondOpFp() const {
            Map<tir::Var, IntImm> bounds = inferBound();
            int n_block = getNumOfBlocks();
            int fp = 0;
        ...
        int IterGraphNode::getSecondOpBlockSize() const{
            Map<tir::Var, IntImm> bounds = inferBound();
            int fp = 0;
            for(auto acf: secondOpReadAccessFuncs)
                for(auto fp_: acf->getFootprint(bounds))
        ...
        FusionResult IterGraphNode::getAnalyticalResult() const{
            return FusionResult(
                inferBound(),\
                getFirstOpFp(),\
                getFirstOpWorkload(),\
        ...
        void IterGraphNode::visualize(){
            ...
            size_t n_tab = 0;
            for(auto common: commonIters){
                for(size_t i = 0; i < n_tab; i++) std::cout << "\t";
                std::cout << "for  " << common->name << " in [0, " << common->ext << "):";
                if(common->shared) std::cout << "\"shared\"";
                std::cout << "\n";
                n_tab++;
            }
            size_t n_tab_ = n_tab;
            for(auto iv: firstOpIters){
                if (iv->shared) continue;
                for(size_t i = 0; i < n_tab_; i++) std::cout << "\t";
        ...
        inline IterGraph buildIterGraph(SerialFusionState sfState)
        {
            ...
            Array<Array<tir::IterVar>> shared_axis = share_axis_analysis(ops1->op, ops2->op);
            ...
            for (auto sharePair : shared_axis)
            {
                ...
                sharedIterPairs.push_back(Share(IterMap.at(sharePair[0]), IterMap.at(sharePair[1])));
            }
            Array<AccessFunction> firstOpReadAccessFunction = ops1->ReadAccessFunctions();
            Array<AccessFunction> secondOpReadAccessFunction = ops2->ReadAccessFunctions();
            ...
            CHECK(readProducerPos >= 0) << readProducerPos << " Can't find a producer for the second op.";
            return IterGraph(firstOpIters,
                             secondOpIters,
                             sharedIterPairs,
                             firstOpReadAccessFunction,
                             secondOpReadAccessFunction,
                             firstOpWriteAccessFunc,
                             secondOpWriteAccessFunc,
                             readProducerPos, 
                             ops1->op, 
                             ops2->op);
        }
        ...
        TVM_REGISTER_GLOBAL("ditto.auto_tensorize.synchronize").\
        set_body_method<IterGraph>(&IterGraphNode::synchronize);
        TVM_REGISTER_GLOBAL("ditto.auto_tensorize.setSchedule").\
        set_body_method<IterGraph>(&IterGraphNode::setSchedule);
