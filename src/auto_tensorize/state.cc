...
Array<tir::Var> OpHyperStateNode::getAllVars(){
  ...
  Array<tir::Var> vars;
  for (auto iv: cop->axis) vars.push_back(iv->var);
  for (auto iv: cop->reduce_axis) vars.push_back(iv->var);
  return vars;
}
...
Array<AccessFunction> OpHyperStateNode::ReadAccessFunctions() {
  ...
  for (auto inp : op->InputTensors()) {
    ...
    for (auto var: allVars){
      ...
      if (isAbsent) absentVars.push_back(var);
      else presentVars.push_back(var);
    }
...
OpHyperState::OpHyperState(te::Operation op, size_t index, Array<tir::IterVar> tensorizeAxes) {
...
SerialFusionState::SerialFusionState(Layer layer, Array<tir::IterVar> tensorizeAxes, std::vector<double> tensorWeight) {
