...
namespace ditto
{
  ...
  namespace auto_tensorize
  {
    ...
    void setTemplatesAndSearchLightWeight(
        ...
        std::vector<FusionInfo> *data = NULL, cost_t * cost_p = NULL, FusionItem * fusionItem_p = NULL)
    {
      Array<IntImm> firstOpPermute, firstOpTiling;
      size_t idx = 0;
      for (auto iv : ig->_firstOpIters)
      ...
      // FusionTemplate fusionTemplates[7] = {
      //     {"F1", {"S1", "S2", "R"}, 3}, {"F2", {"S1", "S2", "R"}, 2}, {"F3", {"S1", "R", "S2"}, 2}, {"F4", {"S2", "R", "S1"}, 2}, {"F5", {"S1", "S2", "R"}, 1}, {"F6", {"S2", "S1", "R"}, 1}, {"F7", {"R", "S1", "S2"}, 1}};
      FusionTemplate fusionTemplates[1] = {
        {"F1", {"S1", "S2", "R"}, 3}
      };
      ...
      std::cout << "cacheLevel_n: " << hw_param->cacheSizePerThread.size() << std::endl;
      ...
      for (size_t fusionLevel = 1; fusionLevel < n_level; fusionLevel++)
      {
        ...
        for (auto fusionTemplate : fusionTemplates)
        {
                        ...
                        &nSecondOpIters](double *occupancy = NULL)
          {
            ...
            return ig->getDM(bytePerEle, false,
                             occupancy);
          };
        ...
        std::cout << "candidates for fusionLevel " << fusionLevel << ": " << n_candidates << std::endl; 
      }
      ...
      std::sort(candidates.begin(), candidates.end(),
                ...
                {
                  if (first.fusionLevel == second.fusionLevel) return first.cost < second.cost;
      ...
      if (data)
      {
        ...
        else if (mode == "survey")
        {
          ...
          for (size_t i = 0; i < candidates.size();
               i += std::max((candidates.size() + SURVEY_CANDIDATES_NUMBER - 1) / (SURVEY_CANDIDATES_NUMBER), (size_t)1))
          {
            FusionInfo fusionInfo = candidates.at(i);
      ...
      if (cost_p) *cost_p = bestCost;
      if (fusionItem_p) *fusionItem_p = bestFusionItem;
                                   ...
                                   String path, int simple_mode)
    {
      ...
      if (simple_mode == 1)
      {
        ...
        bestFusionItem =
            buildFusionItem(vec2array({1, 1, 1, 1}), vec2array({1, 1, 1, 1}),
                            vec2array({0, 1, 2, 3}), vec2array({0, 1, 3, 2}), 4, 3);
      }
      else if (simple_mode == -1)
      {
        ...
        CHECK(m.count(dtype));
        setTemplatesAndSearchLightWeight(ig, hw_param, m[dtype], "normal", "best", NULL, &bestCost, &bestFusionItem);
      }
    ...
    FusionContext::FusionContext(SerialFusionState sfs,
                                 std::vector<CPUTensorizeParam> schParams, Layer layer,
                                 TensorizeHyperFusionState state,
    ...
    std::ostream &operator<<(std::ostream &o, const FusionInfo &  fusionInfo){
      ...
      o << "outerIndices.size(): " << fusionInfo.secondOpOuterIndices.size() << std::endl;
      return o;    
    }
                                     ...
                                     String dtype = "float32")
    {
      ...
      for (auto it: fusionInfo) it.valid = 0;
      ...
      { 
        if (!info.valid) break;
        cnt += 1;
        std::cout << "begin build Tensorize param: " << cnt << "/" << fusionInfo.size() << std::endl;
        CPUTensorizeParam schParam =
            buildCPUTensorizeParam(sfs, hw_param, bytePerEle, info);
        std::vector<double> costs;

        costs.insert(costs.end(), schParam->firstOpCosts.begin(), schParam->firstOpCosts.end());
        costs.insert(costs.end(), schParam->secondOpCosts.begin(), schParam->secondOpCosts.end());
        costs.push_back(info.cost);
        costs.insert(costs.end(), schParam->commCosts.begin(), schParam->commCosts.end());

        schParam->costs = costs;
        double cost = 0;
        for (auto cost_: costs) cost += cost_;
        schParam->cost = cost;
        schParams.push_back(schParam);
      }
      std::sort(schParams.begin(), schParams.end(), [](CPUTensorizeParam &first, CPUTensorizeParam &second)
